...
const MonofasicoPage = () => {
  ...
  const [state3, setState3] = useState({
    ...
    UPS_OPERATIVO:'',
    DESARMADO_DE_EQUIPO:'',
    LA_UNIDAD_PRESENTA_EN_EL_LOG_DE_EVENTOS:'',
    SOBRE_CARGA:'',
    FALLA_DE_BATERIAS:'',
    FALLA_DE_INVERSOR:'',
    FALLA_DE_RECTIFICADOR:'',
    FALLA_DE_BUS_DC:'',
    FALLA_DE_VENTILADORES:'',
    FALLA_DE_BYPASS_ESTATICO:'',
...
const redirectToMenuPage = () => {
  ...
  const generatePDF = () => {
   ...
   // doc.line(100, 100, 100, 200); // 
    ...
    doc.setFontSize(9);
    doc.text(`LOCALIZACIÓN: ${state3.LOCALIZACION}`, 10, 68, { fontSize: 8 });
    doc.text(`UPS OPERATIVO? : ${state3.UPS_OPERATIVO}`, 10, 74, { fontSize: 8 });
    doc.text(`DESARMADO DE EQUIPO UPS : ${state3.DESARMADO_DE_EQUIPO}`, 10, 80, { fontSize: 8 });
    ...
    doc.text(`SOBRE CARGA: ${state3.SOBRE_CARGA}`, 10, 92, { fontSize: 8 });
    doc.text(`FALLA DE BATERIAS : ${state3.FALLA_DE_BATERIAS}`, 10, 98, { fontSize: 8 });
    doc.text(`FALLA DE INVERSOR: ${state3.FALLA_DE_INVERSOR}`, 10, 104, { fontSize: 8 });
    doc.text(`FALLA DE RECTIFICADOR : ${state3.FALLA_DE_RECTIFICADOR}`, 10, 110, { fontSize: 8 });
    doc.text(`FALLA DE BUS DC : ${state3.FALLA_DE_BUS_DC}`, 10, 116, { fontSize: 8 });
    doc.text(`FALLA DE VENTILADORES : ${state3.FALLA_DE_VENTILADORES}`, 10, 122, { fontSize: 8 });
    doc.text(`FALA DE BYPASS ESTÁTICO : ${state3.FALLA_DE_BYPASS_ESTATICO}`, 10, 128, { fontSize: 8 });
